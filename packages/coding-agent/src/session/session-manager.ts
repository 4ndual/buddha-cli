...
export class SessionManager {
	...
	static async open(
		...
		sessionDir?: string,
		storage: SessionStorage = new FileSessionStorage(),
		options?: { initialCwd?: string; suppressBreadcrumb?: boolean; parentSessionID?: string },
	): Promise<SessionManager> {
		...
		const manager = new SessionManager(cwd, dir, true, storage);
		manager.#suppressBreadcrumb = options?.suppressBreadcrumb === true;
		await manager.#setSessionFile(filePath, loaded, { parentSessionID: options?.parentSessionID });
		return manager;
	}
